...
public class Main
{
	public static void main(final String[] args)
	{
		...
		if (cmd.hasOption('n') || cmd.hasOption("number-comics"))
		{
			ncomics = Integer.parseInt(cmd.getOptionValue("number-comics"));
		}
		if (cmd.hasOption('p') || cmd.hasOption("print-titles"))
		{
			printTitles = true;
		}
		if (cmd.hasOption('r') || cmd.hasOption("refresh-cache-file"))
		{
			refreshAnimeClick = true;
		}
		if (cmd.hasOption('f') || cmd.hasOption("cache-file"))
		{
			cacheFile = cmd.getOptionValue("cache-file");
		}
		...
		try
		{
			...
			if (!refreshAnimeClick)
			{
				importer = ComicsImporter.getInstance(cacheFile);
			}
			else
			{
				importer = ComicsImporter.getInstance();
			}
			Comics comics = importer.getComics(ncomics);
